...
// -----------------------------------------------------------------
    ...
    // 2.2 Método el cual retorna el carro que ha estado durante más horas en el parqueadero.
...
public boolean hayCarroMasdeOchoHoras () {
  ...
  public ArrayList<Carro> darCarrosMasDeTresHorasParqueados() {
    ...
    public boolean hayCarrosPlacaIgual() {
        HashSet<String> placas = new HashSet<>(); // Creamos un HashSet para almacenar las placas de los carros, no permiten elementos duplicados, lo que es perfecto para detectar placas repetidas
        for (Puesto puesto : puestos) {
            if (puesto.estaOcupado()) {
                Carro carro = puesto.darCarro();
                if (!placas.add(carro.darPlaca())) {
                    return true;
                }
            }
        }
    ...
    // 3.1 Método que debe retornar el número de carros parqueados cuya placa comienza por “PB”.
    ...
    // 3.2 Método que retorna un valor boolean indicando si hay un carro que lleve más de 24 horas parqueado
    ...
    // 3.4 Método que debe sacar todos los carros del parqueadero y debe retornar la cantidad de carros que fueron sacados
